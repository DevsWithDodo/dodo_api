...
<body>
    <div class="flex-center position-ref">
        <div class="content">
            ...
            <div
                style="background-color: white; color:black; padding:40px;border-radius: 10px;opacity: 0.9;text-align:left; max-width:80%;margin:0 auto;font-size:15px;font-weight:400;text-align:justify">
                Number of users without groups: {{$zero_group}} (not included in the statistics below)<br>
                All: {{$all_users}}<br>
                Guests: {{$guests}}<br>

                Active in the last day/week/month/year/ever: 
                {{$activeLastDay}}/{{$activeLast7}}/{{$activeLast30}}/{{$activeLast365}}/{{$activeEver}}<br>
                ...
                @endforeach
                <br>
                Average number of groups per user: {{$group_avg}}<br>
                Languages:
                ...
                <br>
                <br>
                These stats only include users who have ever been active.
                <br>Light theme vs. dark theme: {{$lightTheme}} / {{$darkTheme}} ({{round($lightTheme / $activeEver * 100)}}% / {{round($darkTheme / $activeEver * 100)}}%)
                <br>Colors used by users which have gradients enabled ({{$users_gradient}}):
                @foreach($colors_gradients_enabled as $color => $count)
                {{$color}} ({{round($count / $users_gradient * 100)}}% / {{$count}})@if (!$loop->last), @endif
                @endforeach
                <br>
                <br>Colors used by free users ({{$users_no_gradient}}):
                ...
                @endif
                @endforeach
                <br>The difference between the free users and the sum of the themes comes from users whose last used theme still was in the free
                trial stage.
            </div>
            @endif
        </div>
