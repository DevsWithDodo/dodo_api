...
Route::get('/groups/{group}/export/get_link_xls', function (\App\Group $group) {
    return URL::temporarySignedRoute('export_xls', now()->addMinutes(1), ['group' => $group, 'language' => auth('api')->user()->language]);
})->middleware(['auth:api']);
...
Route::get('groups/{group}/export/get_link_pdf', function (\App\Group $group) {
    return URL::temporarySignedRoute('export_pdf', now()->addMinutes(1), ['group' => $group, 'language' => auth('api')->user()->language]);
})->middleware(['auth:api']);
